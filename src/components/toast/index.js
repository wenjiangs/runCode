...
Toast.install = function (Vue, options) {
  ...
  Vue.prototype.$toast = function (tips, icon = "") {
    let type;
    var curType = type ? type : opt.defaultType;
    var wordWrap = opt.wordWrap ? 'lx-word-wrap' : '';
    var style = opt.width ? 'style="width: ' + opt.width + '"' : '';
    var tmp = '<div v-show="show" :class="type" class="lx-toast ' + wordWrap + '" ' + style + '>';
    if(icon == 'success'){
      tmp += '<div class="lx-toast-icon lx-toast-icon-success"></div>';
    }else{
      tmp += '<div class="lx-toast-icon lx-toast-icon-error"></div>';
    }
    tmp +=  '{{tip}}' + '</div>';
